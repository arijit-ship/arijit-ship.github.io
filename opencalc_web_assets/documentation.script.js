...
var textIssue = "This is the very first version of this project & I already found some Issues. Hope I will fix them in the next version. You can contribute to the project by fixing them or by suggesting any issues or fixes.<br><br><b>Issue #1 Division by zero Error</b><br><br><mark>X/0</mark> returns <mark>Infinite</mark> instead of <mark>NaN</mark>[ X = Any number except <mark>0</mark>].<br>Issue in MathJS library itself. Need a solution to bypass this.<br><br><b>Issue #2 Trigonometric function argument in degree Error</b><br><br>The function <mark>argumentHandler()</mark> is so broken. I don't know how to fix it. All it should do is replace the argument of trigonometric functions with <mark>argument*dtr</mark> when <i>Deg</i> button is selected in the UI. But this function will NOT work if the input string has multiple same trigonometric functions with the same argument. For example, <mark><i>sin(45)+sin(45)</mark></i>. The function <mark>argumentHandler()</mark> should convert this string into <mark><i>sin(45*dtr)+sin(45*dtr)</i></mark>.But instead what it does is <mark><i>sin(45*dtr*dtr)+sin(45)</mark></i> (As there are two <i>sine</i> functions).<br><mark><i>dtr =  math.pi/180</i></mark>.<br><br><b>Issue #3 <mark>cos 0[in degree]</mark> returns an infinitesimally small number instead of <mark><i>0</i></mark></b><br><br>Probably because of the coefficient <mark><i>dtr</i></mark> to converts the argument from <i>degree</i> to <i>radian</i>.<br><mark><i>dtr =  math.pi/180</i></mark><br><br><b>Issue #4 Avoiding <mark><i>eval()</i></mark> & designing a math parser</b><br><br>At this moment the program depends on <mark><i>function eval()</i></mark>to evaluate the string. However I found that the <mark><i>function eval()</i></mark> is so 'dangerous' and we should avoid it. Also I'd like to design my own math expression parser, but for now I'm ignoring it.<br><br><b>Issue #5 Implementing backspace/delete function in <mark>Clear</mark>button </b><br><br>For now <mark>Clear</mark>button erases everything from display. Click & hold should clear the display and single click should delete one character."
